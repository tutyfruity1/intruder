...
export function sanitizeSettings(input: Partial<SecuritySettings>): SecuritySettings {
  ...
  return {
    ...
    maxRedirects: Math.min(100, Math.max(0, Number.isFinite(input.maxRedirects) ? Math.floor(input.maxRedirects as number) : 10)),
    allowExternalHosts: input.allowExternalHosts !== false,
    ...
    requestTimeout: Math.min(600000, Math.max(0, Number.isFinite(input.requestTimeout) ? Math.floor(input.requestTimeout as number) : 30000)),
    maxResponseSize: Math.min(500 * 1024 * 1024, Math.max(0, Number.isFinite(input.maxResponseSize) ? Math.floor(input.maxResponseSize as number) : 50 * 1024 * 1024)),
